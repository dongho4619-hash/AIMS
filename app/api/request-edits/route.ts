...
export async function GET(request: Request) {
  try {
    ...
    const [item] = await getDb().select({ requesterKey: materialRequests.requesterKey, requester: materialRequests.requester }).from(materialRequests).where(eq(materialRequests.id, requestId)).limit(1);
    if (!item) return Response.json({ error: "신청 건을 찾을 수 없습니다." }, { status: 404 });
    if (item.requesterKey !== user.userId && !(!item.requesterKey && employeeId && item.requester === employeeId)) return Response.json({ error: "본인의 수정 내역만 확인할 수 있습니다." }, { status: 403 });
