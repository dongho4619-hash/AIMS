...
export async function GET(request: Request) {
  try {
    ...
    const users = profile.isAdmin ? await getDb().select().from(appUsers).orderBy(asc(appUsers.employeeId), asc(appUsers.email)) : [];
...
export async function PATCH(request: Request) {
  try {
    ...
    const [updated] = await getDb().update(appUsers).set({ canViewAdmin: payload.canViewAdmin === true, updatedAt: new Date().toISOString() })
      .where(eq(appUsers.userKey, targetUserKey)).returning();
