...
export async function getOrCreateAccess(user: ChatGPTUser, employeeId = ""): Promise<AccessProfile> {
  ...
  const [profileRow] = await db.insert(userProfiles).values({
    userKey: user.userId, employeeId: employeeId.trim(), isAdmin: isOwner, canViewAdmin: isOwner, updatedAt: now,
  }).onConflictDoUpdate({
    target: userProfiles.userKey,
    set: { ...(employeeId.trim() ? { employeeId: employeeId.trim() } : {}), ...(isOwner ? { isAdmin: true, canViewAdmin: true } : {}), updatedAt: now },
  }).returning();
