...
export function Dashboard() {
  ...
  return <main className="service-page">
    ...
    {screen==="inventory"&&<section className="screen-wrap detail-screen"><PageTitle step="04" title="보유자재현황" description="현재 보유재고가 있는 품목만 표시합니다." onBack={()=>go("home")}/><ListTools query={query} setQuery={setQuery} value={category} setValue={setCategory} type="category"/><div className="catalog-summary">보유 품목 <strong>{inventoryMatches.length.toLocaleString()}</strong>개</div><div className="inventory-sheet"><div className="inventory-sheet-head"><span>분류</span><span>품목코드</span><span>품목명 · 규격</span><span>보유재고</span></div>{inventoryMatches.length===0?<div className="empty-box">보유 중인 자재가 없습니다.</div>:inventoryMatches.map(item=><div className="inventory-sheet-row" key={item.sourceKey}><span className="category-cell">{item.category}</span><code>{item.itemCode||"—"}</code><div><strong>{item.itemName}</strong><small>{[item.abbreviation,item.specification,item.notes].filter(Boolean).join(" · ")||"규격 없음"}</small></div><b>{(inventory[item.sourceKey]??0).toLocaleString()}개</b></div>)}</div></section>}
