...
export function Dashboard() {
  ...
  useEffect(()=>{
    ...
    setExpandedInventoryCategories(current=>{
      const visibleNames=inventoryCategoryGroups.map(group=>group.name);
      if(!visibleNames.length) return current;
      return Array.from(new Set([...current,...visibleNames]));
    });
  },[query,inventoryCategoryGroups]);
  useEffect(()=>{
    if(!isNavOpen) return;
