...
authGate.innerHTML = `<form class="auth-card"><span class="detail-kicker">ANYWATER INTEGRATED MANAGEMENT SYSTEM</span><h2></h2><p class="auth-description"></p><label>아이디<input name="username" autocomplete="username" required></label><label>비밀번호<input name="password" type="password" autocomplete="current-password" required></label><label class="display-name-field">관리자 이름<input name="displayName" value="관리자" autocomplete="name"></label><p class="auth-error"></p><button type="submit"></button><p class="auth-caption">계정과 세션은 통합업무관리 전용 DB에 저장됩니다.</p></form>`;
...
authCard.addEventListener("submit", async event => { event.preventDefault(); authError.textContent = ""; const body = Object.fromEntries(new FormData(authCard)); const endpoint = authMode === "setup" ? "/api/auth/setup" : "/api/auth/login"; try { const response = await fetch(endpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "처리하지 못했습니다."); if (authMode === "setup") { authMode = "login"; authCard.querySelector(".display-name-field").hidden = true; authCard.querySelector("h2").textContent = "로그인"; authCard.querySelector(".auth-description").textContent = "관리자 계정이 생성되었습니다. 로그인하세요."; authCard.querySelector("button").textContent = "로그인"; authError.textContent = "관리자 계정이 생성되었습니다."; } else hideAuth(data); } catch (error) { authError.textContent = error.message; } });
